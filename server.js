...
const MONEY_API_KEY = process.env.MONEY_FUSION_API_KEY || "VOTRE_KEY";
const MONEY_SECRET = process.env.MONEY_FUSION_SECRET || "VOTRE_SECRET";
...
app.post("/api/register", (req, res) => {
  ...
  const newUser = { id: uuid(), username, password, balance: 0, totalDeposit: 0 };
  users.push(newUser);
  saveUsers(users);
...
app.post("/api/play", (req, res) => {
  ...
  if (bet < 200 || bet > user.balance) return res.json({ error: "Mise invalide" });
...
app.post("/api/moneyfusion/deposit", async (req, res) => {
  ...
  try {
    // Appel API réel Money Fusion
    const response = await axios.post("https://moneyfusion.example.com/api/deposit", { userId, amount }, {
      headers: { "Authorization": `Bearer ${MONEY_API_KEY}` }
    });
...
app.post("/api/moneyfusion/withdraw", async (req, res) => {
  ...
  try {
    const response = await axios.post("https://moneyfusion.example.com/api/withdraw", { userId, amount }, {
      headers: { "Authorization": `Bearer ${MONEY_API_KEY}` }
    });
